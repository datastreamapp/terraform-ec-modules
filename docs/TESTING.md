...
|--------|-----------|-------|----------|
| `lambda` | `tests/artifact_source.tftest.hcl` | 12 | `artifact_source` variable: local mode, cicd mode, image mode, negative, validation |
...
| 11 | `local_mode_no_source_code_hash` | local | `source_code_hash` not set from `artifact_hash` |
| 12 | `invalid_artifact_source_rejected` | invalid | Validation error |
